...
class HGAScheduler(Scheduler):
    ...
    def create_population(self, app, chromosomes):
        ...
        while chromosomes:
            ...
            for j in range(0, app.tasknum):
                ...
                vrf_sequence.append(chromosome[j+app.tasknum])
    ...
    def crossover(self,pop_size, app, population):
        ...
        for i in range(0, len(population)-1, 2):
            ...
            crossover_point = random.randint(1, 2*app.tasknum-1)
            ...
            crossover_point2 = random.randint(app.tasknum, 2*app.tasknum-1)
            ...
            temp_l.sort(key=lambda seq: seq.tradeoff)
    ...
    def mutate(self,app, population,MutationRate):
        ...
        for p in population:
            if random.random()<MutationRate:
                ...
                temp1=p.chromosome
                temp2=p.chromosome
                pos = random.randint(0, 2*app.tasknum-1)
                if pos < app.tasknum:
                    temp1[pos] = random.randint(0,app.processor_number-1)
                else:
                    temp1[pos] = random.randint(0,len(vrf_set)-1)
                ...
                temp2[pos2] = random.randint(0,len(vrf_set)-1)
                temp.append(temp1)
                temp.append(temp2)
                temp=self.create_population(app,temp)
                if temp[0].tradeoff<=temp[1].tradeoff:
