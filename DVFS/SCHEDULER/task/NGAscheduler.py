...
class NGAScheduler(Scheduler):
    ...
    def init_population(self, app, pop_size):
        ...
        for j in range(0, app.tasknum):
            uprank_tasks.append(random.randint(0, len(vrf_set) - 1))
        ...
        for j in range(0, app.tasknum):
            downrank_tasks.append(random.randint(0, len(vrf_set) - 1))
        ...
        for j in range(0, app.tasknum):
            udrank_tasks.append(random.randint(0, len(vrf_set) - 1))
    ...
    def create_population(self, app, chromosomes):
        ...
        while chromosomes:
            ...
            tsk_sequence=chromosome[:app.tasknum]
            prossor_sequence = []
            vrf_sequence=chromosome[app.tasknum:]
            ...
            allE = consumeEnergyInVRF(processor_set, scheduling_list, makespan)
    ...
    def crossover(self, app,pop_size, population,Elitepop):
        ...
        for i in range(Elitepop, pop_size):
            ...
            crossover_point_dvfs = random.randint(app.tasknum+1, 2*app.tasknum-1)
            ...
            for j in range(app.tasknum-1,-1,-1):
                ...
                if not fd:
                    ...
                    if delta<=crossover_point:
                        break
            for j in range(crossover_point_dvfs,2*app.tasknum):
                next_chromosome1[j],prev_chromosome1[j]=prev_chromosome1[j],next_chromosome1[j]
            ...
            for j in range(app.tasknum - 1, -1, -1):
                ...
                if not fd:
                    ...
                    if delta <= crossover_point:
                        break
            for j in range(crossover_point_dvfs,2*app.tasknum):
                next_chromosome1[j],temp_l[j]=temp_l[j],next_chromosome1[j]
            ...
            for j in range(0,app.tasknum):
                ...
                if not fd:
                    ...
                    if delta >= crossover_point:
                        break
            for j in range(crossover_point_dvfs,2*app.tasknum):
                next_chromosome2[j],prev_chromosome2[j]=prev_chromosome2[j],next_chromosome2[j]
            ...
            for j in range(0,app.tasknum):
                ...
                if not fd:
                    ...
                    if delta >= crossover_point:
                        break
            for j in range(crossover_point_dvfs,2*app.tasknum):
                next_chromosome2[j],temp_l[j]=temp_l[j],next_chromosome2[j]
    ...
    def mutate(self,app,pop_size, population,MutationRate):
        ...
        for p in population:
            if random.random()<MutationRate:
                ...
                pos_dvfs=random.randint(app.tasknum+1, 2*app.tasknum-1)
                p[pos_dvfs]=random.randint(0, len(vrf_set) - 1)
