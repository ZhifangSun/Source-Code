...
class TMGAScheduler(Scheduler):
    ...
    def schedule(self, sign,app,outfilename,target,st):
        ...
        Max_iter=300
        best_ans=[]
        BestFitness = population[0][0].tradeoff
        best_ch = population[0][0]
        for m in range(1,NumOfSubPop):
            if population[m][0].tradeoff+0.000001<BestFitness:
                BestFitness = population[m][0].tradeoff
        ...
        while k < Max_iter:
            ...
            for m in range(NumOfSubPop):
                ...
                next_population.sort(key=lambda seq: seq.tradeoff)
                ...
                NxtSubPop.sort(key=lambda seq: seq.tradeoff)
    ...
    def init_population(self, app, pop_size, NumOfSubPop):
        ...
        for j in range(0, app.tasknum):
            chromosome_uprank.append(random.randint(0, len(vrf_set) - 1))
        ...
        for i in range(NumOfSubPop):
            population[i].sort(key=lambda seq: seq.tradeoff)
    ...
    def GnrChr_Lvl_EFT(self,app):
        ...
        for j in range(0, app.tasknum):
            chromosome.append(random.randint(0,len(vrf_set)-1))
        return chromosome
    ...
    def create_population(self, app, chromosomes):
        ...
        while len(chromosomes) > 0:
            ...
            tsk_sequence = chromosome[:len(chromosome) // 3]

            prossor_sequence = chromosome[len(chromosome) // 3:2*len(chromosome) // 3]

            vrf_sequence = chromosome[2*len(chromosome) // 3:]
            # for ii in vrf_sequence:
            #     print(ii, end=' ')
            # print()
    ...
    def ChrExc(self, app, population, NumOfSubPop, NumOfEliteOfPop, pop_size):
        ...
        EltPop.sort(key=lambda seq: seq.tradeoff)
        ...
        for m in range(NumOfSubPop):
            ...
            NewSubPop.sort(key=lambda seq: seq.tradeoff)
    ...
    def Crs_IL(self,app,ch1,ch2,stg):
        ...
        crossover_point_dvfs = random.randint(2*app.tasknum+1, 3*app.tasknum-1)
        for j in range(crossover_point_dvfs, 3 * app.tasknum):
            ch1[j], ch2[j] = ch2[j], ch1[j]
        return scs,ch1,ch2
    ...
    def MtnSS_IL(self,app,ch):
        ...
        vrf_set = ComputingSystem.VRFs[0]  # 电压频率
        pos2 = random.randint(2*app.tasknum+1, 3 * app.tasknum - 1)
        ch[pos2] = random.randint(0, len(vrf_set) - 1)
        return ch
    ...
    def IFBSI(self,app,ch):
        ...
        IsFrw = not IsFrw
        while NewChrom.tradeoff+0.000001< OldChrom.tradeoff:
            # NewChrom = ch.copy()
            OldChrom = NewChrom.copy()
            ind = self.IndexSort(app, [OldChrom[i].assignment.running_span.finish_time for i in range(len(OldChrom)//2)])
            for i in range(app.tasknum):
    ...
    def IFBSI_LBCRI(self,pop_size,app,population):
        ...
        for i in range(pop_size):
            ...
            for j in range(app.tasknum):
                ...
                if ft[ID] < ans:
                    ft[ID] = ans
            lb = population[i].tradeoff - min(ft)
            population[i].lf = lb
            population[i].ft = ft
        ...
        for i in range(pop_size // 2, pop_size):
            ...
            if new_chr[0].tradeoff < population[i].tradeoff:
